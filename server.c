...
void print_ments()
{
    ...
    printf("|----------- 'exit' 명령어로 SERVER 프로그램을 종료할 수 있습니다. -----------|\n\n");
    printf("+-----------------------------------------------------------------------------+\n\n");
}
		...
		if(quiz_start == 1)
        {
            ...
            else if(receive > 0)
            {
                ...
                if(strlen(buff_out)>0)
                {
                    ...
                    if(strcmp(answer_buf, correct_buf[pc])==0 && correct_m==1)
                    {
                        correct_m = 0;
                    ...
                    {
                        sprintf(buff_out, "[SERVER] %s\'s answer %d is too late!\n", cli->name, cli->answer);
                        locked_send_message(buff_out, 1);
                        printf("%s", buff_out);
                    }
                    //wrong answer
